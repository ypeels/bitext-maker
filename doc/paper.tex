...
% TODO (AMTA padding?): it's rather like having an athlete with an Olympic-level modern training regimen, when it comes to diet/nutrition, they'll just eat whatever they can find on the streets.
% Long and rich tradition of decades, if not centuries of linguists, "chefs" - want to exploit this somehow
% fueled by data
...
% TODO: summarize this in a footnote for emnlp? is there room?
...
& baseline & 11.5 & 0.3 & - & - \\
& system 1 & 10.9 & 0.3 & - & 0.00 \\
\hline
\multirow{2}{*}{TER $\downarrow$}
& baseline & 76.0 & 0.8 & - & - \\
& system 1 & 82.8 & 1.0 & - & 0.00 \\
\hline
\end{tabular}
%\end{footnotesize}
\end{center}
\caption{\label{tab:scores} Metric scores for all systems generated using MultEval 0.5.1 \mycitep{multeval}: jBLEU V0.1.1 (an exact reimplementation of NIST's mteval-v13.pl without tokenization); Translation Error Rate (TER) V0.8.0. p-values are relative to baseline and indicate whether a difference of this magnitude (between the baseline and the system on that line) is likely to be generated again by some random process (a randomized optimizer). Metric scores are averages over multiple runs. $s_{sel}$ indicates the variance due to test set selection and has nothing to do with optimizer instability.}
\end{table}
...
% looks like this is GOING to run over length. save some if not all of this for AMTA, which is going to run way UNDER length
...
BLEU scores were computed as described by \mynewcite{iwslt2015} using the IWSLT 2015 progress data set ({\small \tt tst2014}).\footnote{We chose the {\small \tt tst2014} test set to facilitate comparison with published results from both IWSLT 2014 and 2015 in future work.}

% TODO: mention that LM was held constant - since monolingual (even in-domain?) corpora are typically available at large scale
%\subsection{Task}
