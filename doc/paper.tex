...
\if \buildtarget \buildemnlp    
    ...
    \newcommand\amtaonly[1]{#1} % currently being used to trim down to 4 pages
...
\begin{abstract}
\amtaonly{UNDER CONSTRUCTION}

The large-scale generation of high-quality synthetic parallel corpora for data-driven machine translation is proposed.

A preliminary system for doing this is presented, striving toward syntactic complexity, in the spirit of rule-based machine translation, and high-quality sentence pairs via templates, in the spirit of example-based machine translation.

Early results have yet to show improvement. [This is because we need to scale up?]
Future work is proposed. 
We believe this is particularly amenable to crowdsourcing by ``amateur linguists.''
...
We now consider training set amplification as a first application.
...
The contribution of our synthetic parallel corpora was evaluated using the Chinese to English task of IWSLT 2015 \mycitep{iwslt2015}\emnlponly{.}\amtaonly{, mentioned briefly in Section \ref{subsec:case_study}.}
...
Deep learning methods like RNN encoder-decoders \mycitep{sutskever:2014,bengio:2014} are just the latest in a succession of corpus-based approaches, following in the footsteps of example-based and statistical machine translation, and it is unlikely in this era of Big Data that they will be the last.
But they all share a reliance on parallel corpora, and we believe the task of parallel corpus generation will remain relevant for the foreseeable future.


We have so far been largely concerned with using the computer to increase human translation throughput.
But as alluded to in Section \ref{subsec:benefits}, parallel corpora have the innate potential for open collaboration; not only can they be  expert-sourced to linguists, but they can also crowdsourced to ``amateur linguists.''
...
Multilingual lexical databases, in particular, would be particularly suitable for crowdsourcing, e.g., for slang terms.
...
The data representation (English itself) is widely agreed upon and there is an existing market, a vast untapped resource of student problem sets that is yet to be harnessed by machine learning.
...
The cost per data point could be significantly reduced or perhaps even become negative (a profitable app), although the harvested data itself would clearly have value and would quite possibly be worth subsidizing.
We consider the construction of this potentially game-changing source of supervised NLP data to be an open challenge to the NLP community in general and large industrial practitioners in particular.
