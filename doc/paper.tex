...
Our generated corpora do not make a significant impact on the final BLEU score,\footnote{
    ...
    We believe both to be within the error bars of the MERT tuning procedure.
}
indicating that our generator has a long way to go.

%%%%%%%%%%%%%%%%%%%%%
TODOOOOOOOOOOOO
% TODO: HOW SO?
wanted universal translation memory
much less ambitious goal of training set amplification
- out of domain data only boosts performance slightly
phrase table interpolation using the tmcombine script included with Moses
The results show that our generator has a long way to go.
Quantity and maybe even quality (you'd expect MultiUN translations to be better?) seems to matter much less than whether the additional data is in-domain
So lots of different basic syntactic variation didn't help
vocabulary is much too small? (only about 100 nouns currently. we used Stanford POS tagger \mycitep{postagger:2003} to find most common nouns, since different corpora tend to translate words differently, and the mini-goal here was training set amplification)
more templates
work on that is currently in progress. % I have never liked the sound of ``currently underway''
%%%%%%%%%%%%%%%%%%%%%%%%
...
% no, I can't mention this, because it's just saying, ``duh, hope it's in-domain!''
%For a scaled-up corpus generator, one anticipates that the main concern would be to avoid skewing the phrase table with the ``unnatural'' synthetic corpus.
