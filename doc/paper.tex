...
Preliminary experiments integrating these synthetic corpora into the IWSLT shared task are reported.
In the process, it was incidentally found that correcting $\sim$4\% of the shared task's training corpus for obvious, clerical-type errors resulted in a $\sim$6\% relative increase in BLEU scores.
...
% TODO: preposition (or participle!) might be illustrative, but PP is just WAY too messy and involved to draw in a small space...
% TODO: it would be NICE if the tree matched a YAML example...
% TODO: oh, I could have made a tree diagram using some LaTeX nlp tree package? Word + pdf wound up being pretty fast though
\begin{figure}[ht]
    \centering
    \includegraphics{tree}
    \caption{
        An example tree made of nodes from the {\small \tt nodes} module.
        Solid lines indicate the ``constituent'' backbone that dictates the order of traversal in the {\small \tt generator} module, while dashed lines indicate longer-range syntactic or semantic dependencies that are stored as links between nodes.
        % In this example, V$_1$ can be a ``modalish'' verb like ``want'' that acts like a non-defective modal verb of sorts % meh, let's just bury it in the table below
    }
    \label{fig:tree}
...
For example, it is left to the generator to order prepositional phrases after their targets in English but before their targets in Chinese.
...
% transformations - with scalability in mind, so that things like semantic constraints (<animal> eats <food>) could be used (eating <food>, and should modify <animal>)
...
\begin{table}[ht]
\centering
%\begin{tabular}{|l|l|} % tabularx allows automatic wrapping
\begin{tabularx}{\linewidth}{ |l|X| }
\hline
Clauses & indicative present active; intransitive, transitive, ``meta'' (clause subnode), ``modalish'' (e.g., ``Alice wants to kick Bob'' - with ``Alice'' linked to ``kick'' as in Figure~\ref{fig:tree} for subject-verb semantics) \\ \hline % see with a ``dependency link'' for subject-verb syntactic agreement and 
...
VP & transformations of clauses into participles or modal(ish) complements; convert subject child node into a linked node, allowing reuse of semantic constraints \\ \hline
...
%Custom nodes & ``example-based'' nodes as discussed in Section~\ref{subsec:software} \\ \hline
%Transformations & alterations of existing templates - past tense, topicalization, \zh{把}-construction \\ \hline
\end{tabularx}
\caption{ 
...
Table~\ref{tab:tst2010} below shows the scores obtained by ``lightly'' post-editing the MERT tuning set {\small \tt tst2010} as follows. 
...
\textbf{Warning: This subsection is dedicated to ``data snooping'' experiments that included information from the test set during training in one way or another.
...
It is also worth noting that in a sense, this could regarded as a ``massively-online'' version of elicitation-based techniques from fieldwork linguistics \mycitep{elicitation} for low-resource language pairs, with an emphasis on acquiring unprocessed parallel sentences, and doing so at scale.
}
