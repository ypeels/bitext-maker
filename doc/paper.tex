...
Okay, so our ultimate goal is translation of everyday conversational sentences
...
unlike all the different competing methods, it seems that a ``universal translation memory'' would be inherently collaborative (methods are competitive by nature, can't combine naturally, outside of ensemble methods, whereas data provides a natural avenue for large-scale collaboration)
...
Previous attempts at creating synthetic parallel corpora have used rule-based MT (RBMT) systems \mycitep{hu:2007,dugast:2008,murakami:2009,rubino:2014}


but the input to the RBMT systems was again dictated by external corpora -- domain overlap was prioritized over linguistic consideration of what sentences the RBMT system could translate most accurately.

Our approach, 

not designed from the ground up. We consider a more {\em ab initio} approach
...
But we propose allowing more rule-based modifications of these templates
Also, our goal is to generate a large, wide-coverage, high-quality corpus, not jumping directly to translation from examples as in EBMT systems
