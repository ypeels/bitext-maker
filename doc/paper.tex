...
% code section - do this first, since it MAY shed light on what linguistic phenomena are easy to implement?
    % oh yes, definitely, because then you can explain things briefly in terms of their implementation
    
%Language-specific rules (e.g., word order) are confined to ad hoc bits of code the generator module.\footnote{
%    Formally, this might be considered an extremely rudimentary synchronous (bilingual) context-free grammar of sorts \mycitep{transduction}, one that also admits long-range dependencies and semantic constraints.
...
% maybe use a bulleted list for the different linguistic phenomena supported?
%Clause: transitive, meta, modal
%basic support for NP, ADJP, ADVP (targeting ADJP or Clause), pronouns with antecedents 
%prepositions, which are really relations, allowing semantic constraints on both object and target
%indirect object as a special case of ``null preposition''
%- not necessarily linguistically well-founded, but good enough
%past tenses via transformation of present tense sentence
% other transformations: topicalization, and some hokier stuff like \zh{把} 
%Custom (``template-based mode''): lots of iterals, maybe mark an NP somewhere

% big question: do I want to show a source code example??
%\noindent\makebox[\linewidth]{\rule{\paperwidth}{0.4pt}} % full-width horizontal line
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
